...
def get_volatility(
    ...
    Args:
        close (pd.Series[float]): A pandas series of prices.
    ...
    Returns:
        volatility (pd.Series[float]): A pandas series of volatility values.
...
def get_t_events(close: pd.Series[float], threshold: float) -> pd.DatetimeIndex:
    ...
    Parameters:
        close (pd.Series[float]): Series of close prices.
